...
[[nodiscard]] constexpr auto unpack_nibble(T val) noexcept -> int {
    return (static_cast<std::uint8_t>(val.u8)>>(IDX<<2)) & 0xf;
}
...
#define test_quant_int4(ti, to, rnd) \
    TEST(quantize, quantize_##ti##_to_##to##_##rnd) { \
        std::mt19937 gen {0x9032002}; \
        std::uniform_real_distribution<ti> dist {-1.0, 1.0}; \
        \
        for (std::size_t n {}; n < iters; ++n) { \
            ...
            for (std::size_t i {}; i < numel_out; ++i) { \
                ...
                if (is_int4<to>) { \
                    ...
                    eq |= std::abs(static_cast<int>(data_out[i].u8) - static_cast<int>(data_out_naive[i].u8)) <= stochastic_epsilon; \
                } \
                if (!eq) { \
                    std::cout << "Mismatch at index " << i << ": " << static_cast<int>(data_out[i].u8) << " != " << static_cast<int>(data_out_naive[i].u8) << std::endl; \
                    std::cout << "Input: " << data_in[i] << std::endl; \
                    ASSERT_TRUE(eq); \
                } \
            } \
...
test_quant_int4(float, uint4_t, nearest)
test_quant_int4(float, uint4_t, stochastic)
...
test_quant_int4(float, int4_t, nearest)
test_quant_int4(float, int4_t, stochastic)
...
test_quant_int4(double, uint4_t, nearest)
test_quant_int4(double, uint4_t, stochastic)
...
test_quant_int4(double, int4_t, nearest)
test_quant_int4(double, int4_t, stochastic)
