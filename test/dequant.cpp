...
TEST(dequant, uint8_round_nearest_set) {
    ...
    for (std::size_t n {}; n < iters; ++n) {
        ...
        ctx.quantize_generic<float, std::uint8_t>(data_in, quantized, scale, zero_point, piquant::round_mode::nearest);

        std::vector<float> dequantized {};
        dequantized.resize(numel);
        ctx.dequantize_generic<std::uint8_t, float>(quantized, dequantized, scale, zero_point, piquant::reduce_op::set);
...
TEST(dequant, uint8_round_stochastic_set) {
    ...
    for (std::size_t n {}; n < iters; ++n) {
        ...
        auto [scale, zero_point] {piquant::compute_quant_config_from_data(data_in)};
        piquant::context ctx {std::max(1u, std::thread::hardware_concurrency())};
        ctx.quantize_generic<float, std::uint8_t>(data_in, quantized, scale, zero_point, piquant::round_mode::stochastic);
...
TEST(dequant, uint8_round_nearest_add) {
    ...
    for (std::size_t n {}; n < iters; ++n) {
        ...
        ctx.dequantize_generic<std::uint8_t, float>(quantized, dequantized, scale, zero_point, piquant::reduce_op::add);
        for (std::size_t i {}; i < numel; ++i) {
